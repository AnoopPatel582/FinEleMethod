...
TEST(Q4PlaneStrainModelStiffness, ResolvesElementNodesMaterialAndThickness)
{
    ...
    const auto resolved = q4_plane_strain_stiffness_matrix(element, nodes, materials);
    const Q4NodeCoordinates coordinates{{{{0.0, 0.0}}, {{2.0, 0.0}}, {{2.0, 1.0}}, {{0.0, 1.0}}}};
    const auto expected =
        q4_plane_strain_stiffness_matrix(coordinates, materials.at(9), element.thickness());
...
TEST(Q4PlaneStrainModelStiffness, RejectsElementReferencingMissingNode)
{
    NodeCollection nodes;
    ...
    EXPECT_THROW(static_cast<void>(q4_plane_strain_stiffness_matrix(element, nodes, materials)),
                 std::out_of_range);
}

TEST(Q4PlaneStrainModelStiffness, RejectsElementReferencingMissingMaterial)
{
    NodeCollection nodes;
    nodes.add(Node(1, 0.0, 0.0));
    nodes.add(Node(2, 1.0, 0.0));
