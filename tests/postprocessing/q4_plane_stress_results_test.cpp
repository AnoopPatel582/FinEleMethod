...
using finelemethod::postprocessing::recover_q4_plane_stress_model_results;
...
TEST(Q4PlaneStressModelResults, RecoversEveryElementUsingGlobalDofsAndItsMaterial)
{
    ...
    const auto results = recover_q4_plane_stress_model_results(elements, nodes, materials, dof_map,
    ...
    for (const auto &point : results[1].gauss_points)
    {
        EXPECT_NEAR(point.strain[0], 0.01, tolerance);
        EXPECT_NEAR(point.strain[1], -0.0025, tolerance);
        EXPECT_NEAR(point.stress[0], 20.0, tolerance);
        EXPECT_NEAR(point.von_mises, 20.0, tolerance);
    }
}

TEST(Q4PlaneStressModelResults, RejectsMismatchedGlobalDisplacementSize)
{
    const NodeCollection nodes = make_two_element_nodes();
    const DofMap dof_map(nodes, SpatialDimension::two_dimensional);

    EXPECT_THROW(static_cast<void>(recover_q4_plane_stress_model_results(
                     Q4ElementCollection{}, nodes, MaterialCollection{}, dof_map, DenseVector(11))),
                 std::invalid_argument);
}

TEST(Q4PlaneStressModelResults, RejectsThreeDimensionalDofMap)
{
    const NodeCollection nodes = make_two_element_nodes();
